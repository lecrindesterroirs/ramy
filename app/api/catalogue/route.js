...
export async function POST(request) {
  try {
    ...
    const { prenom, email, entreprise, website } = body
    ...
    if (!prenom || !entreprise || !email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return Response.json({ error: 'Prénom, entreprise et email valides requis.' }, { status: 400 })
    }
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 560px; margin: 0 auto; padding: 40px 32px; background: #FDFBF7;">
          ...
          <p style="font-family: sans-serif; font-size: 15px; line-height: 1.7; color: #333; margin: 0 0 18px;">Bonjour ${esc(prenom)},</p>
          ...
          <p style="font-family: sans-serif; font-size: 15px; line-height: 1.7; color: #333; margin: 0 0 24px;">
            Un projet pour ${esc(entreprise)} ? Répondez simplement à cet email ou appelez-nous au <a href="tel:+33174542310" style="color: #E0A126; text-decoration: none;">01 74 54 23 10</a> : devis personnalisé sous 24&nbsp;h.
          </p>
    ...
    try {
      await resend.emails.send({
        ...
        replyTo: email,
        subject: `Catalogue téléchargé · ${prenom} (${entreprise})`,
        html: `
          <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 32px; background: #FDFBF7;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr><td style="padding: 8px 0; font-family: sans-serif; font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 0.1em; width: 160px;">Prénom</td><td style="padding: 8px 0; font-family: sans-serif; font-size: 14px; color: #1A1A1A;">${esc(prenom)}</td></tr>
              <tr><td style="padding: 8px 0; font-family: sans-serif; font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 0.1em;">Entreprise</td><td style="padding: 8px 0; font-family: sans-serif; font-size: 14px; color: #1A1A1A;">${esc(entreprise)}</td></tr>
              <tr><td style="padding: 8px 0; font-family: sans-serif; font-size: 12px; color: #888; text-transform: uppercase; letter-spacing: 0.1em;">Email</td><td style="padding: 8px 0; font-family: sans-serif; font-size: 14px;"><a href="mailto:${esc(email)}" style="color: #E0A126;">${esc(email)}</a></td></tr>
            </table>
