...
| IA | Anthropic Codex Haiku 4.5 (`Codex-haiku-4-5-latest`), via fetch direct sans SDK |
| Icons | lucide-react |
| Hébergement | Vercel |
...
- Modèle par défaut : `Codex-haiku-4-5-latest`
...
- ❌ Ne pas confondre avec le site web `lecrin-traiteur.fr` (qui est à `/Users/ramy/Desktop/Codex site web` et utilise le skill `lecrin-traiteur`)
...
1. Lire le AGENTS.md / AGENTS.md à la racine de l'OS si présents
...
4. Pour une feature IA : utiliser le skill `Codex-api` pour optimiser (prompt caching)
